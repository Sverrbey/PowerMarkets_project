...
def Main():
    ...
    Data = Read_Excel("Nordic_wet_task35.xlsx")    # Master dictionary, created from input data dictionary

    # Quick check to see that the reference node is valid:
...
# Prepares the datasheet for reading
def Read_Excel(name):
    ...
    data["EmissionCost"] = df["Value"][5]  # Cost of emitting CO2
    data["FutureEmissionCost"] = df["Value"][6]  # Future cost of emitting CO2

    return data        # Return datasheet
# B-matrix, DC-matrix, X-matrix
...
def OPF_model(Data):
    ...
    model.emission = pyo.Param(model.N, initialize=Data["Nodes"]["Emission [gCO2eq/kWh]"])  # Parameter for generation cost for every node
    model.Cost_shed = pyo.Param(initialize=Data["ShedCost"])                  # Parameter for cost of shedding power
    model.Cost_emission = pyo.Param(initialize=Data["EmissionCost"])            # Parameter for cost of emitting CO2
    model.Future_Cost_emission = pyo.Param(initialize=Data["FutureEmissionCost"])   # Parameter for future cost of emitting CO2
    ...
    def ObjRule(model):  # Define objective function
        return sum(model.gen[n]*model.Cost_gen[n] for n in model.N) +\
                ...
                #sum(model.gen[n] * 10**3 * model.emission[n] * model.Future_Cost_emission for n in model.N)
...
def Store_model_data(model, Data):
    ...
    for node in model.N:
        ...
        #Emission[node] = round(model.gen[node].value * 10**3*model.emission[node] * model.Future_Cost_emission, 4)
