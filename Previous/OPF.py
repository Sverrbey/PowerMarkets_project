...
def create_matrices(data):
    ...
    Ybus = np.zeros((nodeNum, nodeNum))  # Create empty matrix of length (n,n)
    for n in range(1, nodeNum+1):   # For every starting node
        for o in range(1, nodeNum+1):   # For every ending node
            for l in range(1, lineNum+1):   # For every line
                ...
                # If starting node corresponds to start in line l
                if translate[a] == data["Transmission line data"]["from"][l]:
                    b = data["Generator data"]['Location'][o]
                    # If ending node corresponds to end in line l
                    if translate[b] == data["Transmission line data"]["to"][l]:

                        # Admittance added in [n-1,o-1]
                        Ybus[n-1][o-1] = Ybus[n-1][o-1] - data["Transmission line data"]["Susceptance [p.u]"][l]
                        # Admittance added in [o-1,n-1]
                        Ybus[o-1][n-1] = Ybus[o-1][n-1] - data["Transmission line data"]["Susceptance [p.u]"][l]

                        # Admittance added in [n-1,n-1]
                        Ybus[n-1][n-1] = Ybus[n-1][n-1] + data["Transmission line data"]["Susceptance [p.u]"][l]
                        # Admittance added in [n-1,n-1]
                        Ybus[o-1][o-1] = Ybus[o-1][o-1] + data["Transmission line data"]["Susceptance [p.u]"][l]
    data["Y"] = Ybus         # Store the matrix in the dictionary

    Ibus = np.zeros((lineNum, nodeNum))  # Dimension [lines, nodes]
    for h in range(1, lineNum + 1):  # For each line
        f = data["Transmission line data"]["from"][h]  # Find the starting node of the line
        t = data["Transmission line data"]["to"][h]  # Find the ending node of the line
    ...
    data["I"] = Ibus  # Store the matrix in the dictionary
    return data
...
def OPF(Data):
    ...
    """ Parameters """
    model.Pu_base = pyo.Param(initialize=1000)
