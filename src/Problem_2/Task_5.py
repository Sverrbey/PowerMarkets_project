...
def DCOPF_model_multiple_gens_and_loads_emissions_CES(N, L, D, G, PGmax, C, demands, U, linecap, susceptance, location_g, location_d, emissions, S_base):
    ...
    def CES(model):
        green_production = sum(model.p_G[g] for g in model.g if model.emissions[g] == 0)
        return  green_production >= 0.20 * sum(model.p_D[d] for d in model.d) 
    model.CES = pyo.Constraint(rule=CES)  
    ...
    # Extract and display CES constraint dual value
    print("\nDual value for CES constraint [NOK/MWh]:")
    dual_value_CES = model.dual.get(model.CES)
    if dual_value_CES is not None:
        print(f"Dual value: {-dual_value_CES/S_base:.2f}")
      
    # Extract and display total emissions
    print("\nTotal emissions [kg]:")
    total_emissions = sum(model.E[g].value for g in model.g)
    print(f"Total emissions: {total_emissions:.2f} kg")



def DCOPF_model_multiple_gens_and_loads_emissions_cap_and_trade(N, L, D, G, PGmax, C, demands, U, linecap, susceptance, location_g, location_d, emissions, S_base):
    model = pyo.ConcreteModel()
    ...
    def emissions_cap(model):
        return sum(model.E[g] for g in model.g) <= 950000
    model.emissions_cap = pyo.Constraint(rule=emissions_cap)
    ...
    # Extract and display the carbon price
    print("\nCarbon price [NOK/kg]:")
    carbon_price = model.dual.get(model.emissions_cap)
    if carbon_price is not None:
        print(f"Carbon price: {carbon_price:.2f}")
    else:
        print("No carbon price found.")

    # Extract and display Dual Value emission per generator
    print("\nDual Value emission per generator [NOK/kg]:")
    for g in model.g:
        dual_value = model.dual.get(model.emissions_rule[g])
        if dual_value is not None:
            print(f"Generator {g}: {dual_value:.2f}")
        else:
            print(f"Generator {g}: No dual value found.")